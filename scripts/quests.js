...
// The progression intentionally ends at the synthetic sapling.  The quest
// system remains fully functional, but late industrial/cosmic checklist content
// no longer distracts from the renewable-world loop.
(() => {
    EMBEDDED_QUEST_DATA.nodes = EMBEDDED_QUEST_DATA.nodes.filter(node => node.id <= 12);
    EMBEDDED_QUEST_DATA.nodes.forEach(node => { node.category = 'Beginning'; });
    EMBEDDED_QUEST_DATA.edges = EMBEDDED_QUEST_DATA.edges.filter(edge => edge.from <= 12 && edge.to <= 12);
    EMBEDDED_QUEST_DATA.nextNodeId = 13;
})();
